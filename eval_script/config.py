...
# config.py 中的 get_parser 函数替换为以下内容：

def get_parser():
    parser = argparse.ArgumentParser(description="Hyperparameters for deep learning training")
    
    #2D or 3D
    #parser.add_argument('--slice_mode', type=int, default=0, help='If 1, use 2D slice mode with 3-axis batching. If 0, use original 3D mode.')
    
    parser.add_argument('--prob_2d', type=float, default=0.5, 
                        help='Probability of using 2D Slice training mode (0.0=Full 3D, 1.0=Full 2D).')
    
    parser.add_argument('--num_slices', nargs='+', type=int, default=[1], 
                        help='Number of slices to extract per axis in 2D mode. Can be a single int or [D, H, W].')
    
    parser.add_argument('--consistency_prob', type=float, default=0.5, 
                        help='Probability that all context slices share the same prompt type in a batch.')
    
    # === 上下文策略超参数 (修改版: 7种组合) ===
    # 纯视觉策略
    parser.add_argument('--prob_a', type=float, default=0, help='概率：Mode A (纯随机上下文)')
    parser.add_argument('--prob_c', type=float, default=0, help='概率：Mode C (全Target上下文)')
    parser.add_argument('--prob_ac', type=float, default=0, help='概率：Mode A + Mode C (即原Mode B: 1个Target + 其余随机)')
    
    # 含文本策略
    parser.add_argument('--prob_text', type=float, default=1, help='概率：Text Only (纯文本，无视觉Context)')
    parser.add_argument('--prob_at', type=float, default=0, help='概率：Mode A + Text (Context随机 + Prompt全Dense)')
    parser.add_argument('--prob_ct', type=float, default=0, help='概率：Mode C + Text (Context全Target + Prompt随机)')
    parser.add_argument('--prob_act', type=float, default=0, help='概率：Mode A + Mode C + Text (即原Mode B+文本)')
    
    # Mode AC/ACT 中 Target 变体对应的 Prompt 类型 (除去 dense)
    parser.add_argument('--mode_b_target_prompt', type=str, default='box', choices=['point', 'box', 'lasso', 'scribble', 'dense_slice'], help='Mode AC/ACT中，作为Context的Target图像使用的Prompt类型')
    ...
    parser.add_argument('--max_slice', type=int, default=None, help = '随机抽取的加入prompt的slice的数量，[1,max_slice-1]')
    parser.add_argument('--grad_checkpointing_base', type=int, default=0, help = '训练时候，是否使用flash attention(A100以上)或者memory efficient (V100)技术降低显存占用。') 
    parser.add_argument('--grad_checkpointing', type=int, default=0, help = '训练时候，是否使用checkpoint技术降低显存占用。') 
    
    # data augmentation 
    parser.add_argument('--flip_prob', type=float, default=0.001, help='Prob. of fliping the seg mask.')
    ...
    # === 新增：动态 Context Size 超参数 ===
    parser.add_argument('--context_size_pool', nargs='+', type=int, default=[], 
                        help='要随机抽取的 context size 候选池 (例如: 1 2 3)。如果为空，则默认不进行动态采样。')
    parser.add_argument('--context_size_probs', nargs='+', type=str, default=['random'], 
                        help='候选池对应的概率 (例如: 0.2 0.3 0.5)。如果传入 "random" 则表示等概率随机均匀抽取。')
...
## 61 RAOS_CT_128: liver, kidney, stomach
    'RAOS_CT_128':[
...
## 62 Dataset680_AMOS22_CT_128: liver, kidney, stomach
    'Dataset680_AMOS22_CT_128':[
...
## 63 FLARE22_seg_5
    'FLARE22_resample_seg_5':[{'name':'Dataset671_FLARE22_LabeledCase50', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [5]},'sample rate':1, 'weight': 1}],
## 64 FLARE22_seg_6
    'FLARE22_resample_seg_6':[{'name':'Dataset671_FLARE22_LabeledCase50', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [6]},'sample rate':1, 'weight': 1}],
## 65 FLARE22_seg_7
    'FLARE22_resample_seg_7':[{'name':'Dataset671_FLARE22_LabeledCase50', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [7]},'sample rate':1, 'weight': 1}],
## 66 FLARE22_seg_8
    'FLARE22_resample_seg_8':[{'name':'Dataset671_FLARE22_LabeledCase50', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [8]},'sample rate':1, 'weight': 1}],
## 67 FLARE22_seg_9
    'FLARE22_resample_seg_9':[{'name':'Dataset671_FLARE22_LabeledCase50', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [9]},'sample rate':1, 'weight': 1}],
## 68 FLARE22_seg_10
    'FLARE22_resample_seg_10':[{'name':'Dataset671_FLARE22_LabeledCase50', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [10]},'sample rate':1, 'weight': 1}],
## 69 FLARE22_seg_11
    'FLARE22_resample_seg_11':[{'name':'Dataset671_FLARE22_LabeledCase50', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [11]},'sample rate':1, 'weight': 1}],
## 70 FLARE22_seg_12
    'FLARE22_resample_seg_12':[{'name':'Dataset671_FLARE22_LabeledCase50', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [12]},'sample rate':1, 'weight': 1}],

## 71 RAOS_CT_128: liver, kidney, stomach
    'RAOS_CT_128_1':[
    {'name':'Dataset683_RAOS_Real_CancerImages_S1_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [1]},'sample rate':1, 'weight': 1},   
    ],

## 72 RAOS_CT_128: liver, kidney, stomach
    'RAOS_CT_128_3_4':[
    {'name':'Dataset683_RAOS_Real_CancerImages_S1_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [3,4]},'sample rate':1, 'weight': 1},   
    ],

## 73 RAOS_CT_128: liver, kidney, stomach
    'RAOS_CT_128_5':[
    {'name':'Dataset683_RAOS_Real_CancerImages_S1_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [5]},'sample rate':1, 'weight': 1},   
    ],

## 74 Dataset680_AMOS22_CT_128: liver, kidney, stomach
    'Dataset680_AMOS22_CT_128_6':[
    {'name':'Dataset680_AMOS22_CT_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [6]},'sample rate':1, 'weight': 1},   
    ],

## 75 Dataset680_AMOS22_CT_128: liver, kidney, stomach
    'Dataset680_AMOS22_CT_128_2_3':[
    {'name':'Dataset680_AMOS22_CT_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [2,3]},'sample rate':1, 'weight': 1},   
    ],

## 76 Dataset680_AMOS22_CT_128: liver, kidney, stomach
    'Dataset680_AMOS22_CT_128_7':[
    {'name':'Dataset680_AMOS22_CT_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [7]},'sample rate':1, 'weight': 1},   
    ],

## 77 Dataset680_AMOS22_CT_128: liver, kidney, stomach
    'Dataset680_AMOS22_CT_128_1':[
    {'name':'Dataset680_AMOS22_CT_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [1]},'sample rate':1, 'weight': 1},   
    ],

## 78  Dataset902_MSD_Prostate
    'Dataset902_MSD_Prostate':[
             {'name':'Dataset902_MSD_Prostate', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [1,2]},'sample rate':1, 'weight': 1},
    ],

## 79  Dataset676_CAS_128
    'Dataset676_CAS_128':[
             {'name':'Dataset676_CAS_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [1]},'sample rate':1, 'weight': 1},
    ],

    

## 80 Dataset680_AMOS22_CT_128:, kidney,
    'Dataset680_AMOS22_CT_128_2':[
    {'name':'Dataset680_AMOS22_CT_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [2]},'sample rate':1, 'weight': 1},   
    ],
## 81 Dataset680_AMOS22_CT_128:, kidney,
    'Dataset680_AMOS22_CT_128_3':[
    {'name':'Dataset680_AMOS22_CT_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [3]},'sample rate':1, 'weight': 1},   
    ],
## 82 RAOS_CT_128:, kidney,
    'RAOS_CT_128_3':[
    {'name':'Dataset683_RAOS_Real_CancerImages_S1_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [3]},'sample rate':1, 'weight': 1},   
    ],
## 83 RAOS_CT_128:, kidney,
    'RAOS_CT_128_4':[
    {'name':'Dataset683_RAOS_Real_CancerImages_S1_128', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [4]},'sample rate':1, 'weight': 1},   
    ],
## 84 NasalSeg:   
    'NasalSeg_1':[
         {'name':'Dataset675_NasalSeg', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [1]},'sample rate':1, 'weight': 1},
    ],
## 85 NasalSeg:   
    'NasalSeg_2':[
         {'name':'Dataset675_NasalSeg', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [2]},'sample rate':1, 'weight': 1},
    ],

# 86 脑区 Thalamus
    'Thalamus_heldout_1Side':[
    {'name':'Dataset725_fcon1000_t1_freesurfer', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [10]},'sample rate':1, 'weight': 1},
    ],      

# 87 脑区Lateral Ventricle
    'Lateral_Ventricle_heldout_1Side':[
    {'name':'Dataset725_fcon1000_t1_freesurfer', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [4]},'sample rate':1, 'weight': 1},
    ],    

# 88 脑区 Putamen
    'Putamen_heldout_1Side':[
    {'name':'Dataset725_fcon1000_t1_freesurfer', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [12]},'sample rate':1, 'weight': 1},
    ],

# 89 Prostate Dataset644_Prostate_RUNMC
    'Dataset644_Prostate_RUNMC':[
    {'name':'Dataset644_Prostate_RUNMC', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [1, 2]},'sample rate':1, 'weight': 1},
    ],

# 90 Prostate Dataset641_Prostate_BMC
    'Dataset641_Prostate_BMC':[
    {'name':'Dataset641_Prostate_BMC', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [1, 2]},'sample rate':1, 'weight': 1},
    ],

# 91 Kidney and Tumor Dataset646_Kits
    'Dataset646_Kits':[
    {'name':'Dataset646_Kits', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [1, 2]},'sample rate':1, 'weight': 1},
    ],
...
## 93  Nasal class 3
    'NasalSeg__3':[
         {'name':'Dataset675_NasalSeg', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [3]},'sample rate':1, 'weight': 1},
    ],

# 94 脑区Hippo
    'Hippo_heldout_17':[
    {'name':'Dataset725_fcon1000_t1_freesurfer', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [17]},'sample rate':1, 'weight': 1},
    #{'name':'Dataset825_fcon1000_t1_freesurfer_raw', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [17]},'sample rate':1, 'weight': 1},
    ],

# 95 脑区 Thalamus
    'Thalamus_heldout_10':[
    {'name':'Dataset825_fcon1000_t1_freesurfer_raw', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [10]},'sample rate':1, 'weight': 1},
    {'name':'Dataset725_fcon1000_t1_freesurfer', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [10]},'sample rate':1, 'weight': 1},
    {'name':'Dataset825_fcon1000_t1_freesurfer_raw', 'input':0,'output':'Seg','task':'Seg','task_config':{'foreground_classes': [10]},'sample rate':1, 'weight': 1},
    ],
