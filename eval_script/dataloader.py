...
#             if mod=='random':
#                 channel = list(config['channel_names'].keys())
#                 mod = random.choice(channel)
...
class MetaDataset_Multi_Extended(MetaDataset_Multi):
    ...
    def __getitem__(self, idx):
        ...
        if not self.in_order: #输出固定的context，用于模型测试
...
class MetaDataset_Multi_Extended_ordered(MetaDataset_Multi_Extended):
    def __init__(self, dataset_dir, 
                        data_loading_config=None, 
                        transform=None, 
                        resize_image=128, 
    ...
    def __getitem__(self, idx):
        ...
        if self.all_idx_relative is None:
            group_size = random.choice([2, 3, 5, 9]) if self.random_context_size and random.random() < 0.05 else self.group_size
            self.all_idx = [random.randint(start_idx, end_idx - 1) for _ in range(group_size)]
        else:
            self.all_idx = [i+idx for i in range(group_size)]
...
class Meta_dataset_Sampler(Sampler):
    ...
    def __next__(self):
        ...
        if self.sample_count >=self.epoch_len:
            # 当生成的样本数达到或超过总样本数时停止迭代
            raise StopIteration
            
            
        if self.count == 0:
            # 如果计数为 0，则选择一个新的随机数并重置计数器
...
if __name__ == "__main__":    
    ...
    dataset = MetaDataset_Multi(
        dataset_dir = '/userhome/jiesi/dataset/', 
